...
init(Id,  Module, Rnd, Sleep) ->
    ...
    %% apply(Module, Function, Args) -> term(). Returns the result of applying Function in Module to Args.
...
init(Id, Module, Rnd, Peer, Sleep) ->
    %% Start a slave process. Start will send a join message.
...
join(Id, Cast) ->
    %% Wait to receive a view after having sent a join messages
    receive
	{view, _} ->
	    Ref = make_ref(),
