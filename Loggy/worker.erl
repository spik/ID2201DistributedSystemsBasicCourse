...
%% While waiting, it is prepared to receive messages from peers so if we run several workers and connect them with each other
%% we will have messages randomly being passed between the workers.
...
loop(Name, Log, Peers, Sleep, Jitter, LamportTime)->
	...
	receive
		{msg, Time, Msg} ->
			...
			NewLamportTime = time:inc(Name, time:merge(LamportTime, Time)),
	...
	after Wait ->
			...
			%% Increment the time
			NewLamportTime = time:inc(Name, LamportTime),
...
%% Returns a random worker from the list of workers
select(Peers) ->
